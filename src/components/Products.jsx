    ...
    <div className="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-8 w-full">
      {products.map((product) => (
        ...
        >
          ...
          <button className="w-1/2 my-4 border-none py-2 font-semibold bg-yellow-500 dark:bg-black hover:scale-105 transition ease-in-out rounded-full">Add to cart</button>
        </div>
      ))}
    </div>
  ) : (<Loader />)
